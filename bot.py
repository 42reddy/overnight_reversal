"""
bot.py  —  Cross-Sectional Overnight-Reversal Basket Bot — Upstox edition
═══════════════════════════════════════════════════════════════════════
...
One trading day looks like:
  ...
  2. At 09:15: pull one bulk LTP snapshot, rank the universe by overnight
     return demeaned cross-sectionally, size the basket (capital split into
     n_splits equal slots, leveraged flat at STRATEGY.intraday_leverage on
     both legs, rounded down to whole shares), and fire one LIMIT entry
     order per name — longs on the biggest losers, shorts on the biggest
     winners.
  3. At 09:20: cancel anything still unfilled. We don't chase.
  4. At 15:20: MARKET-order out of every filled leg (longs SELL, shorts
     BUY-to-cover), confirm fills, and finalize the day's PnL in the trade
     log.

Sandbox vs Live is controlled in config.ini ([SANDBOX] enabled).
...
from auth import get_client, get_analytics_client
...
class ReversalBot:
    ...
    def login(self):
        ...
        order_client, _ = get_client(self.cfg)
        self.api_client = order_client

        # Market data always goes through the long-lived Analytics Access
        # Token (read-only, covers Market Data + Real-time/Streaming APIs)
        # rather than the daily TOTP-login token — works the same in
        # sandbox (which has no market-data API of its own) and live.
        # Orders always go through order_client; the analytics token can't
        # place/modify/cancel orders.
        data_client = get_analytics_client(self.cfg)

        self.engine = SignalEngine(self.cfg, data_client, instruments=self.instruments)
        self.executor = Executor(order_client, self.cfg)
    ...
    def run_exit_pass(self):
        ...
        order_ids = self.executor.place_exits(open_positions)
        ...
        for ticker in unresolved:
            logger.error(f"{ticker}: exit order never confirmed by {deadline} — check the "
                        f"Upstox order book / positions manually.")
...
def run_trading_day(cfg: ConfigParser, capital: float):
    ...
    logger.info("╔══════════════════════════════════════════════╗")
    logger.info("║   Overnight-Reversal Basket Bot — Upstox     ║")
    logger.info("╚══════════════════════════════════════════════╝")
